...
export function useMultiAgentReviewChat(context: () => MeetingContext) {
  ...
  return {
    ...
    userRequestTurn,
    addAgent: async (_data: any) => {},
    setAgentEnabled: meeting.setAgentEnabled,
    deleteAgent: async (_id: string) => {},
    restoreDefaultAgents: async () => { meeting.clearConversation() },
    reorderAgents: (_from: number, _to: number) => {},
    handleInput,
    sendUserMessage,
    setContextElement: meeting.setContextElement,
    updateAgentSettings: async (_id: string, _settings: any) => {},
    approveProposal,
    rejectProposal,
